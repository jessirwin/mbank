...
class variable_handler(object):
	...
	def __init__(self):
        ...
        self.t_formats = ['', 'l1l2'] #tidal layouts 
		...
		for m_, s_, e_, a_, t_ in itertools.product(self.m_formats, self.s_formats, self.e_formats, self.angle_formats, self.t_formats):
				#nonspinning and noneccentric formats don't sample angles...
			#if s_ == 'nonspinning' and (a_ != '' or e_ == ''): continue 
			format_to_add = ''
            # if tidal information included, then BNS 
			for f_ in [m_, s_, t_, e_, a_]:
			...
			self.format_D[format_to_add] = 2 + D_spins[s_] + D_tides[t_] + D_ecc[e_] + D_angles[a_]  #dimension of the variable format
			self.format_info[format_to_add] = {'D':self.format_D[format_to_add],
				'mass_format': m_, 'spin_format': s_, 'tidal_format':t_, 'eccentricity_format': e_,
                'angle_format':a_, 'e': (e_.find('e')>-1), 'meanano': (e_.find('meanano')>-1),
		...
		self.MAX_BBH_SPIN = 0.999 #defining the constant maximum value for the spin (used for any check that's being done)
        self.MAX_Q = 100.
		...
		self.constraints = {'M':(0.,np.inf), 'logM':(-np.inf,np.inf), 'q': (1./self.MAX_Q, self.MAX_Q),
				...
				'chi': (-self.MAX_BBH_SPIN, self.MAX_BBH_SPIN), 
				's1z': (-self.MAX_BBH_SPIN, self.MAX_BBH_SPIN), 's2z': (-self.MAX_BBH_SPIN, self.MAX_BBH_SPIN),
        ...
        # 'Mc':(0., np.inf), 'eta':(1./self.MAX_Q, 0.25),
        # 'mass1':(0, np.inf), 'mass2':(0, np.inf),
        # 'chi': (-self.MAX_BNS_SPIN, self.MAX_BNS_SPIN), 
        # 's1z': (-self.MAX_BNS_SPIN, self.MAX_BNS_SPIN), 's2z': (-self.MAX_BNS_SPIN, self.MAX_BNS_SPIN),
        # #FIXME: you should think VERY VERY carefully on the way you treat the spin variables.
        # #At the moment, mbank cannot cover the negative s1x region in for spin format s1xz
        # 's1': (0., self.MAX_BNS_SPIN), 's2': (0., self.MAX_BNS_SPIN),
        # 'theta1':(-np.pi, np.pi), 'phi1':(-np.pi, np.pi),
        # 'theta2':(-np.pi, np.pi), 'phi1':(-np.pi, np.pi),
        # 'iota': (0.,np.pi), 'phi': (-np.inf, np.inf),
        # 'e': (0., 1.), 'meanano': (0, 1.)
	...
	def labels(self, variable_format, latex = False):
		...
		"""
		assert variable_format in self.valid_formats, "Wrong variable formaaat given"
