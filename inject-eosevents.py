...
fraction_q = torch.tensor(np.array(events.get('Mqlambdatilde'))[1,0,:])
...
M_fractionq_lambdatilde = torch.cat((totalM, fraction_q, lambdatilde))


print(np.shape(M_fractionq_lambdatilde))
exit(0)
...
injs_3D = torch.tensor(Mqlambdatilde)
